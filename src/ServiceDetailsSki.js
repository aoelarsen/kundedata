...
function ServiceDetails() {
  ...
  const handleAddWork = async (e) => {
    const selectedWork = fixedPrices.find(price => price._id === e.target.value && price.serviceType === "Skiservice");
    if (selectedWork) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className={`p-4 border ${isWorkEmpty ? 'border-red-500' : 'border-gray-300'} rounded-lg`}>
          ...
          <select onChange={handleAddWork} className="mt-1 block w-full p-2 border border-gray-300 rounded-md">
            ...
            {fixedPrices
              .filter(price => price.serviceType === "Skiservice")  // Endret fra Sykkelservice til Skiservice
